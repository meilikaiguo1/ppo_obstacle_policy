...
class Voxel3DCNN(nn.Module):
    def __init__(self, in_channels=64, out_dim=256, base_channels=32, dropout_p=0.1):
        super().__init__()
...
class Avoidance_Actor(nn.Module):
...
class Avoidance_Critic(nn.Module):
...
class Avoidance_ActorCritic(nn.Module):
    def __init__(self, obs_self_dim, terrain_feat_dim, action_dim, share_mlp_output_dim):
        super().__init__()
        ...
        self.critic = Avoidance_Critic(256)
    ...
    def get_value(self, obs_self, voxel_grid):
        fused = self.get_features(obs_self, voxel_grid)
        return self.critic(fused)

    def get_action_and_value(self, obs_self, voxel_grid, action=None):
        fused = self.get_features(obs_self, voxel_grid)
