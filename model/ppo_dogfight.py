...
from model.mlp_model import layer_init
import time
...
class DogfightAgent():
    def __init__(self, index, args, trainable = True, torch_dist = False, device = torch.device("cuda:0")):
        ...
        #set save path
        self.save_dir = os.path.join(args.model_dir, str(args.seed), "ActorCritic",  "trained_model")
        ...
        #set network
        self.dogfight_act_dim = args.dogfight_act_dim
        self.dogf_obs_self_dim = args.dogf_obs_self_dim
        self.obs_target_dim = args.obs_target_dim
        self.obs_terrain_dim = args.obs_terrain_dim
        self.mlp_feat_dim = args.mlp_feat_dim

        self.avo_state_dim = args.avo_obs_self_dim
        self.avoidance_act_dim = args.avoidance_act_dim

        self.actor_critic = Dogfight_ActorCritic(self.dogf_obs_self_dim, self.obs_target_dim,self.obs_terrain_dim, self.dogfight_act_dim, self.mlp_feat_dim ).to(self.device)
        self.avoidance_pi = Avoidance_pi(self.avo_state_dim, self.obs_terrain_dim, self.avoidance_act_dim, self.mlp_feat_dim ).to(self.device)
        self.avoidance_pi.eval()
        for p in self.avoidance_pi.parameters():
    ...
    def update(self, data):
        ...
        for epoch in range(self.epoch_train_iters):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                mb_obs_target = b_obs_target[mb_inds].to(self.device, dtype=torch.float32, non_blocking=True)
                ...
                _, new_logp, entropy, new_val = self.actor_critic.get_action_and_value(mb_obs_self, mb_obs_target, mb_voxel_grid, mb_act)
    ...
    def save(self, epoch):
        torch.save(self.actor_critic.state_dict(),os.path.join(self.save_dir, f"dogfight_actor_critic_{epoch}.pt"))
        torch.save(self.actor_critic.actor.state_dict(), os.path.join(self.save_dir, f"dogfight_pi_net_{epoch}.pt"))
        torch.save(self.actor_critic.critic.state_dict(), os.path.join(self.save_dir, f"dogfight_v_net_{epoch}.pt"))
        torch.save(self.ac_optimizer.state_dict(), os.path.join(self.save_dir, f"dogfight_ac_optimizer_{epoch}.pt"))
        torch.save(self.actor_critic.fusion.state_dict(), os.path.join(self.save_dir, f"dogfight_fusion_{epoch}.pt"))