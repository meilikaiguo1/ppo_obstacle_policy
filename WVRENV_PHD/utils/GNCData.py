...
def wgs84ToNED(lat, lon, h, lat0=32.8, lon0=96.3, h0=0):
    """
    :param lat: 当前lla
    :param lon: 当前lla
    :param h: 当前lla
...
def ned_to_wgs84(ned_vec, lat0=32.8, lon0=96.3, h0=0):
    """
    :param ned_vec: NED
    :param lat0: 原点lla
    :param lon0: 原点lla
...
def ned_to_body(vec, yaw, pitch, roll):
    ...
    R_earth2body = r.as_matrix().astype(np.float32)

    if vec.ndim == 1:
        # 单个点: (3,)
        vec = vec.reshape(3, )
        vec_body = R_earth2body @ vec
    elif vec.ndim == 2 and vec.shape[1] == 3:
        # 多个点: (N,3)
        vec_body = vec @ R_earth2body.T
    else:
        raise ValueError(f"vec 的形状不合法，应为 (3,) 或 (N,3)，当前是 {vec.shape}")
...
def body_to_ned(vec, yaw, pitch, roll):
    """
    :param vec: 机体系下的矢量
    :param yaw: 偏航角 角度
    :param pitch: 俯仰角 角度
    ...
    :return:
    """
    r = R.from_euler('XYZ', [-roll, -pitch, -yaw], degrees=True).inv()
    R_body2earth = r.as_matrix()
    vec_ned = np.matmul(R_body2earth, vec)
